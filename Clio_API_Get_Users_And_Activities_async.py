...
async def get_users(url: str, session: OAuth2Session) -> list[dict]:
    """
    ...
    while resp.json()['meta']['paging'].get('next'):
        ...
        if resp.headers.get('Retry-After'):
            ...
            await asyncio.sleep(retry_period + 1)
...
async def get_calendar_entries(url: str, session: OAuth2Session) -> list[dict]:
    """
...
async def main() -> None:
    ...
    # Async get calendar entries and users
    calendar_task = asyncio.create_task(get_calendar_entries(base_calendar_url, session))
    user_task = asyncio.create_task(get_users(base_users_url, session))

    # Get values from calendar and user async tasks
    users = await user_task
    calendar_entries = await calendar_task
